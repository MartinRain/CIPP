...
**Shared mailboxes, calendars and sites**
...
| SharePoint Sites           | The SharePoint sites the new user should be added to. Several can be selected.                                                                               |
| SharePoint Site Role       | The access the user is given on those sites: `Members`, `Owners` or `Visitors`. Defaults to `Members`.                                                       |
...
A newly created user is not a usable Exchange recipient for the first few minutes, and takes a similar time to reach SharePoint, so the mailbox, calendar and site grants are all queued as scheduled tasks that run 15 minutes after creation. Their progress, and any failure, can be followed on the [scheduler](../../../tools/scheduler/ "mention")page.
{% endhint %}
