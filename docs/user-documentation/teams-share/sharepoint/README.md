...
This page lists the SharePoint sites in the selected tenant, with the storage each one is consuming, the number of files it holds, and when it was last used. The list includes Teams channel sites, the separate sites Teams creates for private and shared channels, alongside the tenant's other sites. From here you can manage who has access to a site, change its sharing and storage settings, clean up old file versions, work through its recycle bin, and delete sites you no longer need.
...
A storage bar above the table shows how much of the tenant's overall SharePoint storage pool is in use, with the used, free and total figures alongside. These figures come from the tenant's quota in SharePoint rather than from usage reporting, so they are unaffected by report anonymisation. They reflect the quota as of CIPP's last collection of the tenant's data, and the bar does not follow the page's switch between live and cached data. The bar appears when a single tenant is selected and you have SharePoint administration read access; if the quota cannot be read, it is replaced with a note that tenant storage usage is unavailable.
